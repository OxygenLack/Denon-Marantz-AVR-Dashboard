...
function defaultConfig(info) {
  const saved = info?.night_mode_config || {}
  return {
    mode: saved.mode || 'offset',
...
export default function NightModeModal({ open, onClose, state, info, post, onConfigChange }) {
  ...
  const [schedule, setSchedule] = useState(defaultConfig(info).schedule)
  ...
  useEffect(() => {
    ...
    const globalMode = cfg.mode === 'absolute' ? 'absolute' : 'offset'
    setMode(globalMode)
