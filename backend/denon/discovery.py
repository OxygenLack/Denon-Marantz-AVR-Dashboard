...
async def discover_receivers(timeout: float = 4.0) -> list[dict[str, Any]]:
    ...
    # A configured host must also appear in discovery results. This matters in
    # bridge-mode containers where SSDP multicast and LAN subnet scanning are
    # not available even though the receiver itself is reachable.
    configured_host = settings.denon_host.strip()
    if configured_host:
        telnet_ok = await loop.run_in_executor(
            None, _probe_port, configured_host, settings.denon_telnet_port, 1.0
        )
        if telnet_ok:
            ...
            seen[configured_host] = {
                ...
                "telnet_port": settings.denon_telnet_port,
                "heos_available": heos_ok,
            }
            _LOGGER.info("Discovery: added configured receiver %s", configured_host)
        else:
            _LOGGER.warning(
                "Configured receiver %s is not reachable on telnet port %d",
                configured_host,
                settings.denon_telnet_port,
            )
